...
for m = 1,#models do
  model = torch.load('model_' .. m .. '.t7'):double()
...
torch.save('outputs_all.t7', outputs)
print('<outputs_all> saved')

torch.save('outputs_avg.t7', outputs[#models+1])
print('<outputs_avg> saved')

torch.save('outputs_ensemble.t7', outputs[#models+2])
print('<outputs_ensemble> saved')

torch.save('outputs_geo.t7', outputs[#models+3])
print('<outputs_geo> saved')

torch.save('soft_targets.t7',outputs[#models+4])
